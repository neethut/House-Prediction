...
public class Main {
	...
	public Main(){
		...
		submit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				JLabel label11=new JLabel("No. of rooms: "+bestPossibleConf.get(1).toString());
				JLabel label12=new JLabel("Distance from Boston's top 5 employment centres: "+bestPossibleConf.get(2).toString());
				JLabel label13=new JLabel("The Highway access Index: "+bestPossibleConf.get(3).toString());
				JLabel label14=new JLabel();
				if(bestPossibleConf.get(4)>15){
					label14.setText("Educational facilities: Very Good; in terms of pupil-teacher ratio: "+bestPossibleConf.get(4).toString());
				}
				else{
					label14.setText("Educational facilities: Good; in terms of pupil-teacher ratio: "+bestPossibleConf.get(4).toString());
				}
				JLabel label15=new JLabel("Tax: $"+bestPossibleConf.get(5));
				JLabel label16=new JLabel("And, budget for this house is: $"+bestPossibleConf.get(6)*1000);
				...
				if(bestPossibleConf.get(7)>3){
					label17.setText("Warning! The per capita crime rate in the area is reported high at:"+bestPossibleConf.get(7).toString() );
					panelOut.add(label17);
				}
				/*JLabel label10=new JLabel("The house that utmost satisfies your need has following features:");
				JLabel label11=new JLabel("price: "+bestPossibleConf.get(1).toString());
				panelOut.add(label10);
				panelOut.add(label11);
				*/
	...
	private ArrayList<Float> computeCosineSimilarity(ArrayList<Float> userPref) {
		System.out.println(userPref);
		ArrayList<Float> availableHouse=new ArrayList<>();
		ArrayList<HousePrice> cosSimilarity=new ArrayList<>();
		ArrayList<Float> bestHouseConfig=new ArrayList<>();
		float budgetrangeUp;
		...
		session = cluster.connect("house_plan");
		
		float  userData=userPref.get(0);
		...
		String q="SELECT * FROM house where riverside=0 and price <"+budgetrangeUp+"and price>"+budgetrangeDown+" allow filtering";
		ResultSet results = session.execute(q);
		
		float price,similarity,id;
		
		for (Row row : results) {
			price=row.getFloat("price");
			if (price<budgetrangeUp && price > budgetrangeDown) {
				availableHouse.add(row.getFloat("rooms"));
				availableHouse.add(row.getFloat("riverside"));
				availableHouse.add(row.getFloat("built_b4"));
				availableHouse.add(row.getFloat("weighted"));
				availableHouse.add(row.getFloat("highway"));
				availableHouse.add(row.getFloat("pupil_ratio"));
				availableHouse.add(row.getFloat("price"));
				id=row.getFloat("id");
				similarity=cosineSimilarity(userPref,availableHouse);
				HousePrice housePrice = new HousePrice(id,similarity);
				cosSimilarity.add(housePrice);
				availableHouse.clear();
			}
			//System.out.println("cossimilarity:"+cosSimilarity.toString());
			
		}
		System.out.println("cossimilarity\n"+cosSimilarity.toString());
		bestHouseConfig=computeBest5Houses(cosSimilarity);
		cluster.close();
	
		return bestHouseConfig;
	}
	
	private ArrayList<Float> computeBest5Houses(ArrayList<HousePrice> cosSimilarity) {
		...
		float max=cosSimilarity.get(0).similarity;
		
		id=cosSimilarity.get(0).id;
		for(int i=1;i<cosSimilarity.size();i++){
			if(cosSimilarity.get(i).similarity>max){
				max=cosSimilarity.get(i).similarity;
				id= cosSimilarity.get(i).id;
			}
		}
		
		float rooms = 0,industrial=0,highway=0,pupil_ratio=0,tax=0,crim=0;
		String q="SELECT * FROM house where id="+id;
		ResultSet results = session.execute(q);
		
		for (Row row : results) {
			price = row.getFloat("price");
			rooms=row.getFloat("rooms");
			industrial=row.getFloat("industrial");
			highway=row.getFloat("highway");
			pupil_ratio=row.getFloat("pupil_ratio");
			tax=row.getFloat("tax");
			crim=row.getFloat("CRIM");
		}
		
		bestHouseConfig.add(id);
		bestHouseConfig.add(rooms);
		bestHouseConfig.add(industrial);
		bestHouseConfig.add(highway);
		bestHouseConfig.add(pupil_ratio);
		bestHouseConfig.add(tax);
		bestHouseConfig.add(price);
		bestHouseConfig.add(crim);
		
		return bestHouseConfig;
	}
	
	private float cosineSimilarity(ArrayList<Float> userPref2,ArrayList<Float> availableHouse) {
		// TODO Auto-generated method stub
		float numerator=0;
		float denominator=0;
		float temp1=0;
